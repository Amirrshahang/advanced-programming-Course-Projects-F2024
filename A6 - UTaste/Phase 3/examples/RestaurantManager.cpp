...
string RestaurantManager::getRestaurantsByProximity(const string& userDistrict) {
    string result;
    ...
    if (!allRestaurants.empty()) {
        for (auto& restaurant : allRestaurants) {
            result += restaurant.getName() + " (" + restaurant.getDistrict() + ")<br>";
        }
    }
...
string RestaurantManager::reserveTable(const string& restaurantName, int tableId, int startTime, int endTime, const string& username, const vector<pair<string, int>>& orderedFoods, UserManager& userManager) {
    ...
    if (userManager.decreaseWallet(username, finalPrice)) {
        ...
        result << "Reserve ID: " << reserveId << "<br>";
        result << "Table " << tableId << " for " << startTime << " to " << endTime << " in " << restaurantName << "<br>";
        result << "Original Price: " << totalPrice << "<br>";
        if (totalPrice != 0) {
            if (itemDiscount > 0) {
                result << "Total Item Specific Discount: " << itemDiscount << "<br>";
            }
            ...
            if (discount.totalPriceDiscount.has_value()) {
                ...
                if (totalPriceDiscountValue > 0) {
                    result << "Order Amount Discount: " << totalPriceDiscountValue << "<br>";
                }
            }
            ...
            if (discount.firstOrderDiscount.has_value() && totalDiscount > itemDiscount && isFirstReservation) {
                ...
                result << "First Order Discount: " << firstOrderDiscountValue << "<br>";
            }

            result << "Total Discount: " << totalPrice - finalPrice << "<br>";
            result << "Total Price: " << finalPrice << "<br>";
        }
    } else {
...
string RestaurantManager::showAllUserReservations(const string& username) {
    ...
    for (const auto& [id, restaurant, tableId, startTime, endTime, foods, totalPrice, finalPrice] : reservations) {
        result << id << ": " << restaurant << " " << tableId << " " << startTime << "-" << endTime;

        if (!foods.empty()) {
            for (const auto& [foodName, count] : foods) {
                result << " " << foodName << "(" << count << ")";
            }
        }

        result << " " << totalPrice << " " << finalPrice << "<br>";
    }
