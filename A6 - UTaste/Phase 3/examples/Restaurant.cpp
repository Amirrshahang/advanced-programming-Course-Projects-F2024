...
string Restaurant::printRestaurantDetails() const {
    ostringstream result;

    result << "Name: " << name << "<br>";
    result << "District: " << district << "<br>";
    result << "Time: " << opening_time << "-" << closing_time << "<br>";
    result << "Menu: ";
    ...
    for (size_t i = 0; i < sortedFoods.size(); ++i) {
        result << sortedFoods[i].first << "(" << sortedFoods[i].second << ")";
    ...
    for (const auto& [tableNumber, reservations] : tables) {
        ...
        for (size_t i = 0; i < sortedReservations.size(); ++i) {
            result << "(" << sortedReservations[i].first << "-" << sortedReservations[i].second << ")";
